...
- **Spine sizing initially ported the mockup's exact formula** (`design.html:705-706`): `height = 148 + (hash % 65)`, `width = 32 + (hash % 9)` px. **Superseded during Phase 2, iterated live with the user (2026-08-20, commit `39e4b28`):** the fixed formula truncated long titles too aggressively at real content lengths (the mockup only ever shows ~10 short titles). Shipped behavior: height is still hash-derived within the same `65`px spread, but off a title-length-aware floor (`SPINE_MIN_HEIGHT`, `src/lib/spine-style.utils.ts`), and titles past 35 characters split into two rendered columns (`splitTitleForTwoColumns`) instead of clipping. See `src/lib/spine-style.utils.ts` for the current formula; this bullet is left in place as a record of the original intent it deviated from.
- **Palette and dark-text conditional were both widened during Phase 2, iterated live with the user (2026-08-20, commit `39e4b28`).** Originally: `spinePalette`'s last entry (`#ECF5EC`, a near-white green) was to trigger `design.html`'s `.on-paper` class via `color === palette[palette.length - 1]`. Shipped instead: the mockup's 8-color palette collided too often across a real collection's title count, so `SPINE_PALETTE` widened to the app's full 12-token green/blue scale (`src/lib/spine-style.utils.ts`), and the dark-text conditional generalized from a single hardcoded entry to a relative-luminance threshold (`onPaper = relativeLuminance(color) > 0.6`) so every light entry in the wider palette gets dark text, not just the original one.
...
- **Own-book `statusSlot` is not `undefined` — Phase 2's Contract text was incomplete, not the code (found in impl review, 2026-08-21).** The Phase 2 "Changes Required" section says `statusSlot` stays `undefined` for own books, on the grounds that no availability pill is needed. What it missed: the pre-change `book-row.tsx` also rendered `book.notes` and the active-loan line (`Lent to Ania · since 12 Mar 2026`) inline, and no other slot in the Drawer's contract has anywhere to put them. `book-row.tsx` correctly passes `statusSlot={book.notes || book.loan ? <div>...notes/loan...</div> : undefined}` — reusing `statusSlot` for this rather than adding a slot the Drawer primitive doesn't have. Treat the Phase 2 Contract's "`statusSlot` stays `undefined`" as narrowed to "no availability pill" going forward, not literal `undefined`.
- **Owner name added to the Discover drawer post-hoc (impl review finding F1, 2026-08-21).** Phase 3's drawer contract (title/author/isbn/statusSlot/actionsSlot) never mentioned the book's owner, and neither the row nor the drawer displayed it — a regression from the pre-change Card row, which showed "Owned by {name}". Since `/discover` defaults to showing all friends' books on one shelf, this made ownership unrecoverable from the UI. Fixed by adding an `owner?` prop to `Spine` (folded into its `aria-label`/tooltip only, no visible spine label) and an "Owned by {name}" line inside the discover drawer's `statusSlot`, above the availability `Pill`. `book.notes` (also dropped from the discover row, unlike collection's) was left dropped — a deliberate scope call, not an oversight.
...
- [x] 2.1 Type checking passes: `npx tsc --noEmit` — 39e4b28
- [x] 2.2 Linting passes: `npm run lint` — 39e4b28
- [x] 2.3 Full suite passes: `npm test` — 39e4b28
- [x] 2.4 Production build passes: `npm run build` — 39e4b28

#### Manual

- [x] 2.5 Shelf renders, spine colors/sizes correct, drawer opens with correct info (incl. no-ISBN case), Edit opens EditBookModal, Delete works (incl. loan-guard) — 39e4b28
- [x] 2.6 Layout holds at 375px — 39e4b28
...
- [ ] 3.5 Shelf renders per friend, search narrows it, spine tags correct for all 4 availability states, drawer status/action correct per state, borrow works end-to-end (reopened by impl review F7, 2026-08-21 — previously checked off on "jest ok," not a browser pass; re-verify now that F1–F6 fixes have touched Spine/Drawer/Shelf/discover/page.tsx)
- [ ] 3.6 Layout holds at 375px (reopened alongside 3.5, same reason)
